...
public class LiveStreamRepository {
    ...
    public LiveStream findById(String id) throws LiveStreamNotFoundException {
        return streams.stream().filter(stream -> stream.getFirstName().equals(id)).findFirst().orElseThrow(LiveStreamNotFoundException::new);
    }
    ...
    public void update(LiveStream stream, String id) {
        LiveStream existing = streams.stream().filter(s -> s.getFirstName().equals(id))
                .findFirst()
    ...
    public void delete(String id) {
        streams.removeIf(stream -> stream.getFirstName().equals(id));
    }
